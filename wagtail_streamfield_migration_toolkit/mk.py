...
def intersectiono(xs, ys, zs):
    @goal
    def intersectiono_goal(state: State) -> Stream:
        return disj(
            eq(xs, Nil()) & eq(zs, Nil()),
            eq(ys, Nil()) & eq(zs, Nil()),
            fresh(
                lambda a, d, res: conj(
                    ...
                    ifte(
                        ...
                        conj(
                            eq((a, res), zs),
                            intersectiono(d, ys, res),
                        ),
                        intersectiono(d, ys, zs),
                    ),
                ),
            ),
        )(state)

    return intersectiono_goal


def insert_manyo(xs, ys, zs):
    """
    maybe think about it like this:
    - if head(ys) == head(zs), xs does not change
    - otherwise, we must have an element in zs that was inserted into ys
    """

    @goal
    def insert_manyo_goal(state: State) -> Stream:
        return disj(
            eq(xs, Nil()) & eq(ys, zs),
            eq(ys, Nil()) & eq(xs, zs),
            eq(zs, Nil()) & eq(xs, Nil()) & eq(ys, Nil()),
            fresh(
                lambda ay, dy, az, dz: conj(
                    eq((ay, dy), ys),
                    eq((az, dz), zs),
                    eq(ay, az),
                    insert_manyo(xs, dy, dz),
                ),
            ),
            fresh(
                lambda az, dz, res: conj(
                    eq((az, dz), zs), inserto(az, xs, res), insert_manyo(res, ys, dz)
                ),
            ),
            # fresh(
            #     lambda a, d, res: conj(
            #         # I don't think xs is being constrained enough if it is initially fresh
            #         # if it is, eq((a, d), xs) will always succeed
            #         # so maybe we need another clause here
            #         eq((a, d), xs),
            #         inserto(a, ys, res),
            #         # in this case a remains fresh after inserto call
            #         insert_manyo(d, res, zs),
            #     ),
            # ),
        )(state)

    return insert_manyo_goal
